...
//
// CB INDICES — see alpha_blend_partial_host.h for the canonical declaration. The
...
void kernel_main() {
    ...
    // CB indices — must match alpha_blend_partial_host.h verbatim. Compute kernels
    ...
    for (uint32_t t = 0; t < num_tiles; t++) {
        ...
        // ----- Per-segment finalize: pack the running R/G/B accumulators AND
        // the running transmittance T to CB_COLOR_OUT in a single 4-tile push.
        // This is the partial op: each job composites a depth-SEGMENT starting
        // from T=1, so we must emit T (not just RGB) for the combine op to merge
        // segments via the associative `over` operator. The writer DMAs the 4
        // tiles (R,G,B,T) to partials[4*partial_slot + {0,1,2,3}].
        tile_regs_acquire();
        ...
        copy_tile_to_dst_init_short(CB_T_STATE);
        copy_tile(CB_T_STATE, 0, 3);
        tile_regs_commit();
        tile_regs_wait();
        cb_reserve_back(CB_COLOR_OUT, 4);
        pack_tile(0, CB_COLOR_OUT);
        pack_tile(1, CB_COLOR_OUT);
        pack_tile(2, CB_COLOR_OUT);
        pack_tile(3, CB_COLOR_OUT);
        cb_push_back(CB_COLOR_OUT, 4);
        tile_regs_release();
